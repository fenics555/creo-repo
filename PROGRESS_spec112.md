...
### ПРОВЕРКА PDF И РАСХОЖДЕНИЕ БИБЛИОТЕКИ (22.09.2026, вечер, нога Cline)
- Ф0: `Test-Path D:\AI\repo\Creo\CREO_DOCS` → **False**; семь имён PDF не найдены ни в `D:\AI\ИЗУЧИТЬ\CREO` (обход), ни в `repo\data\backup` (обход); `git ls-files Creo/CREO_DOCS` → 7 файлов в индексе, в статусе `D`; ветка `[ahead 28, behind 25]`, `ls-remote origin master` → `91556ce`.
- ВЕТКА B СЛОВА: удаления зафиксированы адресно — коммит `3075d54` «CREO_DOCS pdfs removed by user word 22.09» (7 delete-mode, путь ограничен `Creo/CREO_DOCS`). В отчёт: «потеря принята пользователем, не нарушение: удаление рукой хозяина».

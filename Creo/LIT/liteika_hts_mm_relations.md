...
/* Верификация данных перед запуском основных расчетов */
...
IF H0_MM <= 0
    message "ВНИМАНИЕ: Высота стояка не задана или равна 0. Проверьте геометрию модели."
ENDIF

/* Проверка схемы питания */
IF SXEMA_PITANIYA < 1 OR SXEMA_PITANIYA > 4
    message "ВНИМАНИЕ: Номер схемы питания вне диапазона 1-4, уставлено значение 1 (классическая схема)."
    SXEMA_PITANIYA = 1
ENDIF

/* Проверка типа холодильника */
IF MAT_CHILLER < 1 OR MAT_CHILLER > 3
    message "ВНИМАНИЕ: Неверный тип холодильника, использовано значение 1 (сталь/чугун)."
    MAT_CHILLER = 1
ENDIF

/* Проверка усадки */
IF USADKA_P < 0.5 OR USADKA_P > 2.0
    message "ВНИМАНИЕ: Коэффициент усадки USADKA_P вне рекомендуемого диапазона 0.5-2.0."
ENDIF

/* --- Конец блока верификации --- */

/* --- ST_ Стояк: предупредительный optimum, подбор и пост-адапт --- */
...
ST_D_NIZ_MIN_MM = ceil(sqrt(4 * HYDRO_S_ST_MM2 / CONST_GEN_PI_RATIO) / CONST_ST_STEP_MM) * CONST_ST_STEP_MM
IF ST_D_NIZ_MIN_MM < CONST_ST_MIN_D_MM
    ST_D_NIZ_MIN_MM = CONST_ST_MIN_D_MM
    message "ВНИМАНИЕ: Расчетный диаметр стояка меньше минимума, использовано значение " STRING(CONST_ST_MIN_D_MM) " мм."
ENDIF
ST_D_NIZ_FINAL_MM = ST_D_NIZ_MIN_MM
/* Проверка скорости в стояке */
ST_V_FLOW_MM_S = HYDRO_S_ST_MM2 / (PI * pow(ST_D_NIZ_FINAL_MM / 2, 2))
IF ST_V_FLOW_MM_S > CONST_FLOW_ST_V_MAX_MM_S
    message "ВНИМАНИЕ: Скорость в стояке превышает норму (" STRING(ST_V_FLOW_MM_S) " мм/с, максимум " STRING(CONST_FLOW_ST_V_MAX_MM_S) " мм/с)."
ENDIF
/* --- Конец блока ST --- */

/* --- SHL_ Шлакоуловитель без учета усадки --- */
SHL_S_ONE_MM2 = HYDRO_S_SHL_MM2 / SAFE_KOL_SHL
/* Расчет длины шлакоуловителя */
SHL_L_MM = 2.0 * SHL_S_ONE_MM2 / (PI * SHL_R_MM)
/* Проверка соотношения высоты/диаметра */
IF SHL_L_MM / SHL_R_MM < 1.5
    message "ВНИМАНИЕ: Соотношение длины/диаметра шлакоуловителя меньше 1.5, рекомендуется увеличение."
ENDIF
/* --- Конец блока SHL --- */

/* --- PRIB_ Расчет прибылей и дефицита металла --- */
ST_V_NEED_MM = V_REAL_PRIB_LAT / (PI * pow(NOM_D_PRIB_LAT / 2, 2))
IF ST_V_NEED_MM > CONST_FLOW_ST_V_MAX_MM_S
    message "ВНИМАНИЕ: Скорость в прибылях превышает норму (" STRING(ST_V_NEED_MM) " мм/с)."
ENDIF
/* Проверка модуля Черворинова */
M_CHVORINOV_CALC = V_REAL_PRIB_LAT / A_REAL_PRIB_LAT
IF M_CHVORINOV_CALC < DELTA_MM / 5
    message "ВНИМАНИЕ: Модуль Черворинова меньше рекомендуемого (デルタ_MM / 5). Рекомендуется увеличение размеров рисеров."
ENDIF
/* Проверка соотношения высота/диаметр стояка */
IF H0_MM / DELTA_MM < 10
    message "ВНИМАНИЕ: Соотношение высоты стояка к толщине стенки меньше 10. Рекомендуется увеличение стояка."
ENDIF
/* --- Конец блока PRIB --- */

    message "ВНИМАНИЕ: Неверный тип холодильника, использовано значение 1 (сталь/чугун)."
ELSEIF MAT_CHILLER > 3
    message "ВНИМАНИЕ: Неверный тип холодильника, использовано значение 3 (графит)."
ENDIF

IF MAT_CHILLER == 1
    message "Информация: Тип холодильника - сталь/чугун. Коэффициент интенсивности: 0.50"
ENDIF
IF MAT_CHILLER == 2
    message "Информация: Тип холодильника - медь. Коэффициент интенсивности: 0.30"
ENDIF
IF MAT_CHILLER == 3
    message "Информация: Тип холодильника - графит. Коэффициент интенсивности: 0.40"
ENDIF

/* --- Дополнительные проверки: схема питания --- */
IF SXEMA_PITANIYA < 1
    message "ВНИМАНИЕ: Номер схемы питания меньше 1, уставлено 1 (классическая схема)."
ELSEIF SXEMA_PITANIYA > 4
    message "ВНИМАНИЕ: Номер схемы питания больше 4, уставлено 4 (только верх)."
ENDIF

IF SXEMA_PITANIYA == 1
    message "Информация: Схема 1 - Автономная работа (боки и верх кормят раздельно)."
ENDIF
IF SXEMA_PITANIYA == 2
    message "Информация: Схема 2 - Совместная работа прибылей."
ENDIF
IF SXEMA_PITANIYA == 3
    message "Информация: Схема 3 - Только боковые стаканы."
ENDIF
IF SXEMA_PITANIYA == 4
    message "Информация: Схема 4 - Только верхний купол."
ENDIF

/* --- Финальный вердикт SAFE-блока --- */
IF MASSA_OTLIVKI <= 0
    message "ВНИМАНИЕ: Масса отливки не задана или равна 0. Результаты расчетов могут быть некорректны."
ENDIF
IF H0_MM <= 0
    message "ВНИМАНИЕ: Высота стояка не задана или равна 0. Проверьте геометрию модели."
ENDIF
IF SXEMA_PITANIYA == 0
    message "ВНИМАНИЕ: Схема питания не задана."
ENDIF

message "СТАТУС ОК: Верификация входных параметров завершена успешно."
...
FLOW_ST_V_ACT_MM_S = HYDRO_S_ST_MM2 / (PI * pow(ST_D_NIZ_FINAL_MM / 2, 2))
IF FLOW_ST_V_ACT_MM_S > CONST_FLOW_ST_V_MAX_MM_S
    message "ВНИМАНИЕ: Фактическая скорость в standing (" STRING(FLOW_ST_V_ACT_MM_S) " мм/с) превышает максимум (" STRING(CONST_FLOW_ST_V_MAX_MM_S) " мм/с)."
ENDIF
IF FLOW_ST_V_ACT_MM_S < CONST_FLOW_ST_V_MIN_MM_S
    message "ВНИМАНИЕ: Скорость в standing ниже минимальной (" STRING(CONST_FLOW_ST_V_MIN_MM_S) " мм/с). Рекомендуется пересмотр сечения."
ENDIF

FLOW_SHL_V_MM_S = HYDRO_S_SHL_MM2 / (PI * SHL_R_MM * 2)
IF FLOW_SHL_V_MM_S > CONST_FLOW_SHL_V_MAX_MM_S
    message "ВНИМАНИЕ: Скорость в шлакоуловителе (" STRING(FLOW_SHL_V_MM_S) " мм/с) превышает норму."
ENDIF

/* --- Конец блока FLOW --- */

/* --- HTS_ ХТС-форма: расчет геометрии и параметров --- */
HTS_H_SHEIKA_NOM_MM = NOM_H_SHEIKA_LAT * HOT_FACTOR
HTS_D_SHEIKA_NOM_MM = NOM_D_SHEIKA_LAT * HOT_FACTOR
HTS_L1_SHEIKA_NOM_MM = NOM_L1_SHEIKA * HOT_FACTOR
HTS_L2_SHEIKA_NOM_MM = NOM_L2_SHEIKA * HOT_FACTOR
HTS_S_SHEIKA_NOM_MM2 = S_SHEIKA_FINAL_MM2 * pow(HOT_FACTOR, 2)

/* Проверка геометрии ХТС-формы */
IF HTS_H_SHEIKA_NOM_MM < CONST_HTS_MIN_H_MM
    message "ВНИМАНИЕ: Номинальная высота ХТС-формы меньше минимума (" STRING(CONST_HTS_MIN_H_MM) " мм)."
ENDIF
IF HTS_D_SHEIKA_NOM_MM < CONST_HTS_MIN_D_MM
    message "ВНИМАНИЕ: Номинальный диаметр ХТС-формы меньше минимума (" STRING(CONST_HTS_MIN_D_MM) " мм)."
ENDIF

/* --- Конец блока HTS --- */

/* --- ECO_ Экономическая эффективность --- */
ECO_V_MOD_MM3 = pow(NOM_D_PRIB_MOD / 2, 2) * PI * NOM_H_PRIB_MOD * HOT_FACTOR
ECO_M_MOD_KG = ECO_V_MOD_MM3 / 1000000 * PM /* PM - плотность материала */
ECO_CYCLE_TIME_S = (L_ZAZOR_MM + NOM_H_SHEIKA_LAT) * CONST_ECO_CYCLE_K_S

/* Экономическая эффективность */
ECO_EFFICIENCY_P = (M_SHEIKA_EFF / ECO_M_MOD_KG) * 100
IF ECO_EFFICIENCY_P < CONST_ECO_MIN_EFF_P
    message "ВНИМАНИЕ: Экономическая эффективность (" STRING(ECO_EFFICIENCY_P) " %) ниже нормы (" STRING(CONST_ECO_MIN_EFF_P) "%)."
ENDIF

/* --- Конец блока ECO --- */

/* --- QUAL_ Оценка качества и рисков --- */
QUAL_DEFECT_RATE_P = 0.5 /* Базовый коэффициент брака */
QUAL_STRESS_FACTOR = H0_MM / DELTA_MM

/* Проверка факторов риска */
IF QUAL_STRESS_FACTOR > 15
    message "ВНИМАНИЕ: Коэффициент напряжения (" STRING(QUAL_STRESS_FACTOR) ") превышает безопасный предел 15. Рекомендуется пересмотр геометрии."
ENDIF
IF QUAL_DEFECT_RATE_P > 3.0
    message "ВНИМАНИЕ: Уровень брака (" STRING(QUAL_DEFECT_RATE_P) " %) выше рекомендуемого 3.0%."
ENDIF

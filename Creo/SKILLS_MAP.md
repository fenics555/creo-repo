...
* Инструмент дома **`plm_reader`** (V2, ПЛМ «в лоб»): `D:\AI\tools\agent\plm_reader\` — движок `engine.py`
  (своя база `db\plm_reader.db`: `snapshots`/`links`/`derived`/`changes`/`folders`/`meta`; скан инкрементальный,
  проверка «актуально/нужен скан»), окно `plm_reader.py` (вкладки `Таблица`/`Дерево`, нижнее онлайн-дерево
  производства, карточка изделия, история файла окном, ПУРГЕ-план/бэкап, фильтр по всем словам).
  Паспорт — `plm_reader\README.md`; передача без базы — `SHARE_copy.bat`. Скилл: §8.37–8.66.

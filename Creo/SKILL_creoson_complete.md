...
system: Creo
description: Use when: любая работа с Creo через CREOSON â имена команд, параметры, JSON-формат, сценарии
...
- Деталь â .prt, сборка â .asm, чертёж â .drw, формат â .frm, сечение â .sec,.layout â .lay.
- На диске к имени добавляется версия: korpus.prt.3; в командах CREOSON файл указывается БЕЗ версии: korpus.prt.
- Расширения .cre НЕ СУЩЕСТВУЕТ â не выдумывать. Команды Â«CREOSON RENAMEÂ» в bash НЕ СУЩЕСТВУЕТ.
...
- Истина по CREOSON â СЫРОЙ ответ живого сервера (TEST-first), а не справка. PTC-хелп (support.ptc.com/help/creo) â только в последнюю очередь.

# CREOSON â ПОЛНАЯ КАРТА API (сервер 3.0, JSON поверх JLINK)
...
CREOSON Server  http://127.0.0.1:8080/creoson
  |  JLINK (PTC API)
Creo Parametric (3.0+, лучше всего 8â12)
...
- sessionId берётся ОДИН РАЗ через connection:connect и переиспользуется во всех запросах;
  при ошибке со словом "session" â реконнект и повтор.
...
-- connection --
connect {} â sessionId (ПЕРВЫМ вызовом)
disconnect {}
is_creo_running {} â {"running": true}
...
regenerate {"file"}
list {} â файлы в сессии (data.file_list / filelist / files)
exists {"file"}
get_active {} â data строкой или {"file": ...}
get_fileinfo {"file"}
display | erase | erase_not_displayed | close_window {"file"}
assemble {"files":[...]}
backup {"file","dirname"}
massprops {"file"} â mass, volume, area/surface_area, density
open_errors {"file"}
...
-- creo --
pwd {} â рабочая папка (data = строка ИЛИ {"directory"})
cd {"dirname"}
list_files {"filename": маска} â data.file_list / files
list_dirs | mkdir | rmdir | delete_files
get_config {"name"} â data.values (список значений)
set_config {"name","value"}
get_std_color | set_std_color
set_creo_version {"version": 12}  â ОБЯЗАТЕЛЬНО для Creo 7+ (иначе падают file:regenerate, file:assemble, feature:*, familytable:replace)

-- parameter --
list {"file"} â data.param_list (name, type, value, designate, description)
...
â дерево: file, children[], seq_path (root.3.2); НЕ суй children списком в walk â корень это dict
...
mapkey {"script"}  â тяжёлая артиллерия для того, чего CREOSON не умеет
...
-- server --
pwd â папка сервера CREOSON

== 4. СЦЕНАРИИ (выверены) ==

Подключение (первым вызовом):
    POST /creoson  {"command":"connection","function":"connect","data":{}}
    â sessionId во все следующие запросы.

Открыть + масса:
    file:open {"file":"korpus.prt","display":true}
    file:massprops {"file":"korpus.prt"} â data.mass (кг)

Параметры:
    parameter:list {"file":"korpus.prt"} â data.param_list
...
Умное переименование (деталь + чертёж + сборки):
    ...
    (в агенте â один инструмент creo_rename_model)

PDF:
    interface:export_pdf {"file":"chertezh.drw","dirname":"Z:/PDF_out","use_drawing_settings":true}

Чистка склада версий:
    creo:list_files â группы имя.prt.N â удалить все кроме последней
    (в агенте â creo_purge_versions)

Аудит папки:
    creo:pwd â creo:list_files â для каждого .prt/.asm:
    file:open â parameter:list (ОБОЗНАЧЕНИЕ/НАИМЕНОВАНИЕ/MASS) â file:relations_get
    â file:erase, если не была открыта до аудита
    (в агенте â creo_audit_folder)
	
== PDF-ГЛАЗА: свежесть и перепечать ==
Вердикт свежести считается НЕ через Creo: mtime одноимённых pdf и drw из таблицы files
(та же папка, тот же стем); pdf >= drw â актуален, drw новее â УСТАРЕЛ; mtime prt/asm не учитывается.
...
"top_level":false} â корень дерева dict, children не списком. Экземпляры семейств для плана
копии/переименования: file:has_instances / file:list_instances + familytable:list_tree;
порядок переименования â по SKILL_copy_rename.	
...
Для остального â interface:mapkey с готовым mapkey-скриптом.
# ---

## Manufacturing API (Creo Manufacturing â не Creoson)

- Manufacturing в Creo â модуль NC, Molde, Wire EDM, и т.д., и это не стандартный Creoson API.
- Manufacturing данные: спецификации, каталоги деталей, параметры â доступны через Creo Toolkit или Creo Manufacturing API.
- Для Manufacturing используйте Creo-native инструменты (Creo Manufacturing, Creo Simulate).
- Эти API не входят в Creoson. Если нужно Manufacturing â то это отдельная задача.

## CAE (Creo Simulate / Mechanica â не Creoson)

- CAE (расчёты нагрузок, анализ напряжений) â это модуль Creo Simulate / Mechanica, не Creoson.
- Для работы с CAE используйте Creo Simulate API или Creo Toolkit.
- Нагрузки: силы, давления, моменты, тепловые â доступны через Creo Simulate.
- Если нужно CAE â то это отдельная задача (или Creo Simulate API / Creo Toolkit).
...
- file:path, file:parents, file:children â не существуют
- creo:get_title, creo:get_info, credo:rebuild â не существуют
- bom:copy, bom:list â не существуют
- group:list â не существует (Invalid command: group)
- pdf:isinstance, pdf:isprintable â не существуют (Invalid command: pdf)
- file:rename требует параметр new_name, а не new (живой ответ: "No 'new_name' parameter given")
- file:rename с new_name возвращает "General Error" â требует проверки условий успеха (модель открыта, нет блокировок)

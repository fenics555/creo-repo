...
ПРОФИЛАКТИКА: раздел «ПРОТОКОЛ ДЕЙСТВИЙ» выше — остановка только по PID из pid-файла/netstat, никогда по имени; после рестарта — приёмка портов.

## ПОВТОРЫ:
- Если после рестарта агент снова "застыл" в течение 5 минут — СТОП. Выдать отчёт о повторном краше и ждать указаний.
- ПЕРВЫЙ ФИКС: 1 (счётчик по шаблону конституции; первый случай — 19.09.2026).

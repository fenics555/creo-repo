...
**Дом:** CREO-AGENT v15 · **Дата паспорта:** 14 сентября 2026 · **Рантайм:** D:\AI\tools\agent · **Git-корень кода:** D:\AI\tools · **Git-корень знаний:** D:\AI\repo · **Контракт:** D:\AI\.clinerules v5.6
...
- Модули инструментов: `graph_tools.py`, `timeline_tools.py`, `vision_audit.py`, `map_tools.py`, `similar_tools.py`, `calc_tools.py` (угол), и ещё тридцать три модуля (backup, behavior, chat, creo_ops, creo, db, diagnostic, draft, find, fleet, git, help, knowledge, learn, memory, nightly, one_c, passport, pdf, plm, predict, role, scanner, settings, spec, sync, trail, usage, users, vision, web)
...
- **Эмбеддинги:** `nomic-embed-text:latest` — 768-мерный вектор, индексация по текстовым корням, 34 000+ чанков, 25 628 моделей с эмбеддингами
...
8. **search_kb** — семантический поиск по 34 000+ чанкам через косинусное сходство
...
- **33** — витрина и справка (в процессе: кнопка «подробнее», вычистка рассуждений, инвентаризация ключей)
...
**Куда положить:** `D:\AI\repo\PASSPORT.md`, один write_file целиком (потолка 3 КБ на ответ в чате нет, на write_file — есть; если упираешься — разбить на три куска по шаблону из мини-спеки DOCS-1). После — `git -C D:\AI\repo add PASSPORT.md && git -C D:\AI\repo commit -m "passport: полный паспорт по состоянию на 14.09.2026" && git -C D:\AI\repo push`.

Состояние дома после закрытия эпохи ремонтов и открытия эпохи роста: дом говорит по-русски, видит чертежи глазами, помнит 34 тысячи чанков знаний, умеет находить похожие модели, знает связи и время, держит флот машин КБ, и растёт через инструменты. Список будущего — только косметика витрины (33) и дальнейший аппетит.

...
**Дом:** CREO-AGENT v15 · **Дата паспорта:** 16 сентября 2026 · **Обновлён:** 16 сентября 2026 (спека 44v) · **Рантайм:** D:\AI\tools\agent · **Git-корень кода:** D:\AI\tools · **Git-корень знаний:** D:\AI\repo · **Контракт:** D:\AI\.clinerules v5.6
...
**Куда положить:** `D:\AI\repo\PASSPORT.md`. После правки — `git -C D:\AI\repo add PASSPORT.md && git -C D:\AI\repo commit -m "passport: обновлён 16.09.2026 (спека 44v)" && git -C D:\AI\repo push`.

Состояние дома после завершения эпохи ремонтов и перехода к фазе активного роста: дом говорит по-русски, видит чертежи глазами, помнит 34 365 фрагментов знаний, умеет находить похожие модели, знает связи и время, держит флот машин КБ и растёт через инструменты (38 блоков, 133 инструмента). Спека 33 (витрина) закрыта, спека 34 (ремонт эпохи роста) закрыта, спека 44v (автономия цикла) закрыта. Дальше — только аппетит.

- 16.09.2026: правило вылетов ред.2 — крахи исполнителей регистрируются скилами в
  D:\AI\repo\ВЫЛЕТЫ\ (шаблон в SKILL_crash_protocol_v2.md); продолжение не запрещено,
  повторы считаются, 3+ выносится пользователю; периодическая систематизация ВЫЛЕТОВ —
  отдельная задача.